...
namespace UHFDemo
{
    public partial class R2000UartDemo : Form
    {
        ...
        public static DataGridView tablaTiempos { get; set; }
        public List<int> idCategoriasCarrera { get; set; }
        public List<string> categoriasCarrera { get; set; }
        ...
        public void R2000UartDemo_Load(object sender, EventArgs e)
        {
            LlenarComboCarreras();
            LlenarListadoCarreras();
            LlenarListadoCategorias();
            LlenarListadoCompetidores();
            //初始化访问读写器实例
        ...
        public void LlenarListadoCategorias()
        {
            ...
            try
            {
                ...
                DataGridView grid = tablaCategorias;
                grid.Rows.Clear();
                while (reader.Read())
                {
                    int indice =0 ;
                    ...
                    string[] rowNuevo = new string[] { idCategoria.ToString(), descripcion };
                    grid.Rows.Add(rowNuevo);
                }

                tablaCategorias = grid;
            }
            catch (Exception ex)
            {
        ...
        public void LlenarListadoCompetidores()
        {
            string connectionString = "SERVER=localhost;DATABASE=atletica;UID=root;PASSWORD=pecopeco1290;";
            MySqlConnection mysqlCon = new MySqlConnection(connectionString);
            ...
            try
            {
                ...
                sentencia.AppendLine("  CONCAT(A.ApellidoPaterno, ' ', A.ApellidoMaterno, ' ' , A.Nombre) as NombreCompleto, ");
                sentencia.AppendLine("  A.Direccion, ");
                sentencia.AppendLine(" 	B.Descripcion AS Pais, ");
                sentencia.AppendLine(" 	C.Descripcion AS Estado, ");
                sentencia.AppendLine("  D.Descripcion AS Ciudad, ");
                sentencia.AppendLine(" 	A.Telefono, ");
                sentencia.AppendLine(" 	A.FechaNacimiento ");
                sentencia.AppendLine(" FROM ");
                sentencia.AppendLine(" 	competidores AS A ");
                sentencia.AppendLine(" LEFT JOIN ");
                sentencia.AppendLine(" 	paises AS B ");
                sentencia.AppendLine(" ON B.IdPais = A.IdPais ");
                sentencia.AppendLine(" LEFT JOIN ");
                sentencia.AppendLine(" 	estados AS C ");
                sentencia.AppendLine(" ON	C.IdPais = A.IdPais ");
                sentencia.AppendLine(" AND C.IdEstado = A.IdEstado ");
                sentencia.AppendLine(" LEFT JOIN ");
                sentencia.AppendLine(" 	ciudades AS D ");
                sentencia.AppendLine(" ON 	D.IdPais = A.IdPais ");
                sentencia.AppendLine(" AND D.IdEstado = A.IdEstado ");
                sentencia.AppendLine(" AND D.IdCiudad = A.IdCiudad ");
                ...
                MySqlCommand cmd = new MySqlCommand(query, mysqlCon);
                MySqlDataReader reader = cmd.ExecuteReader();
                DataGridView grid = tablaCompetidores;
                grid.Rows.Clear();
                ...
                while (reader.Read())
                {
                    ...
                    string direccion = (reader[indice] is DBNull) ? string.Empty : reader.GetString(indice); indice++;
                    string pais = (reader[indice] is DBNull) ? string.Empty : reader.GetString(indice); indice++;
                    string estado = (reader[indice] is DBNull) ? string.Empty : reader.GetString(indice); indice++;
                    string ciudad = (reader[indice] is DBNull) ? string.Empty : reader.GetString(indice); indice++;
                    string telefono = (reader[indice] is DBNull) ? string.Empty : reader.GetString(indice); indice++;
                    DateTime fechaNacimiento = (reader[indice] is DBNull) ? DateTime.MinValue : reader.GetDateTime(indice); indice++;

                    string[] rowNuevo = new string[] { idCompetidor.ToString(), nombreCompleto, direccion, pais, estado, ciudad, telefono, fechaNacimiento.ToString("dd/MM/yyyy")};
                    grid.Rows.Add(rowNuevo);
                }

                tablaCompetidores = grid;
        ...
        public void LlenarListadoCarreras()
        {
            string connectionString = "SERVER=localhost;DATABASE=atletica;UID=root;PASSWORD=pecopeco1290;";
            MySqlConnection mysqlCon = new MySqlConnection(connectionString);
            ...
            try
            {
                ...
                sentencia.AppendLine("  	A.Descripcion as DescripcionCarrera, ");
                ...
                string sqlSelectAll = sentencia.ToString();
                MyDA.SelectCommand = new MySqlCommand(sqlSelectAll, mysqlCon);
                MyDA.Fill(table);

              
                tablaCarreras.DataSource = table;
        ...
        public void RefreshInventoryReal(byte btCmd)
        {
            btCmdGlobal = btCmd;
            if (this.InvokeRequired)
            {
                    ...
                    case 0x8B:
                        {
                            ...
                            DataRow renglon = m_curInventoryBuffer.dtTagTable.Rows[nEpcLength - 1];
                            if (renglon != null && renglon[2] != null)
                            {
                                
                                if (Globales.chipActual != renglon[2].ToString().Trim())
                                {
                                    Globales.chipActual = renglon[2].ToString().Trim();
                                }
        ...
        public void btRealTimeInventory_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                if (m_curInventoryBuffer.lAntenna.Count == 0)
                {
                    MessageBox.Show("Porfavor seleccione por lo menos una antena ");
                    return;
                }
                //默认循环发送命令
                if (m_curInventoryBuffer.bLoopInventory)
        ...
        {
            VistaCapturaCategorias vista = new VistaCapturaCategorias(this);
            vista.RegistrarCategoria();
            vista.ShowDialog();          
        }
        ...
        private void btnModificarCategoria_Click(object sender, EventArgs e)
        {
            ...
            {
                DataGridViewRow row = this.tablaCategorias.SelectedRows[0];
                int idCategoria = row.Cells["IdCategoria"] != null && !string.IsNullOrEmpty(row.Cells["IdCategoria"].Value.ToString()) ? Convert.ToInt32(row.Cells["IdCategoria"].Value) : 0;
                string descripcion = row.Cells["Descripcion"] != null && !string.IsNullOrEmpty(row.Cells["Descripcion"].Value.ToString()) ? row.Cells["Descripcion"].Value.ToString() : "";

                VistaCapturaCategorias vista = new VistaCapturaCategorias(this);
                vista.ModificarCategoria(idCategoria, descripcion);
                vista.ShowDialog();
            }
            else
            {
        ...
        private void btnEliminarCategoria_Click(object sender, EventArgs e)
        {
            ...
            if (tablaCategorias.SelectedRows.Count != 0)
            {
                DialogResult result = MessageBox.Show("¿Desea eliminar el registro seleccionado?", "Confirmación", MessageBoxButtons.YesNo);
                if (result == DialogResult.Yes)
                {
                    ...
                    try
                    {
                        ...
                        mysqlCon.Open();
                        string query = "DELETE FROM CATEGORIAS WHERE IDEMPRESA = " + IdEmpresa + " AND IDCATEGORIA = " + idCategoria;
                        MySqlCommand cmd = new MySqlCommand(query, mysqlCon);
                        ...
                        {
                            MessageBox.Show("Error al eliminar registro, favor de comunicarse con administrador.");
                        }
                    }
                    catch (Exception ex)
                    {
        ...
        private void btnRegistrarCompetidor_Click_1(object sender, EventArgs e)
        {
            VistaCapturaCompetidores vista = new VistaCapturaCompetidores(this);
            vista.RegistrarCompetidor();
            vista.ShowDialog();
        }

        private void btnModificarCompetidor_Click_1(object sender, EventArgs e)
        {
            if (tablaCompetidores.SelectedRows.Count != 0)
            {
                DataGridViewRow row = this.tablaCompetidores.SelectedRows[0];
                int idCompetidor = row.Cells[0] != null && !string.IsNullOrEmpty(row.Cells[0].Value.ToString()) ? Convert.ToInt32(row.Cells[0].Value) : 0;

                VistaCapturaCompetidores vista = new VistaCapturaCompetidores(this);
                vista.ModificarCompetidor(idCompetidor);
                vista.ShowDialog();
            }
            else
            {
                MessageBox.Show("Debe de seleccionar un competidor");
            }
        }

        private void btnEliminarCompetidor_Click_1(object sender, EventArgs e)
        {
            string connectionString = "SERVER=localhost;DATABASE=atletica;UID=root;PASSWORD=pecopeco1290;";
            MySqlConnection mysqlCon = new MySqlConnection(connectionString);
            if (tablaCompetidores.SelectedRows.Count != 0)
            {
                ...
                {
                    DataGridViewRow row = this.tablaCompetidores.SelectedRows[0];
                    int idCompetidor = Convert.ToInt32(row.Cells[0].Value);

                    try
                    {

                        mysqlCon.Open();
                        string query = "DELETE FROM COMPETIDORES WHERE IDEMPRESA = " + IdEmpresa + " AND IDCOMPETIDOR = " + idCompetidor;
                        MySqlCommand cmd = new MySqlCommand(query, mysqlCon);

                        if (cmd.ExecuteNonQuery() > 0)
                        {
                            MessageBox.Show("Registro eliminado correctamente");
                            tablaCompetidores.Rows.Remove(row);
                        }
                        else
                        {
                            MessageBox.Show("Error al eliminar registro, favor de comunicarse con administrador.");
        ...
        private void btnRegistrarCarrera_Click(object sender, EventArgs e)
        {
            ...
            vista.accion = "registrar";
            vista.RegistrarCarrera();
            vista.Show();
        ...
        {
            if (tablaCarreras.SelectedRows.Count != 0)
            {
                ...
                vista.ModificarCarrera(idCarrera);
                vista.Show();
        ...
        public void LlenarComboCarreras()
        {
            string connectionString = "SERVER=localhost;DATABASE=atletica;UID=root;PASSWORD=pecopeco1290;";
            MySqlConnection mysqlCon = new MySqlConnection(connectionString);
            string query = "SELECT IdCarrera, Descripcion FROM Carreras WHERE IdEmpresa = '" + IdEmpresa + "'";
            try
            {
                ...
                {
                    int indice = 0;
                    int idCarrera = (reader[indice] is DBNull) ? 0 : reader.GetInt32(indice); indice++;
                    string descripcion = (reader[indice] is DBNull) ? string.Empty : reader.GetString(indice); indice++;

                    lista.Add(new Item(descripcion, idCarrera));
                }

                cmbCarreraConfig.DisplayMember = "Descripcion";
                cmbCarreraConfig.ValueMember = "IdCarrera";
                cmbCarreraConfig.DataSource = lista;
                cmbCarreraConfig.SelectedIndexChanged += cmbCarreraConfig_SelectedIndexChanged;
        ...
        {
            Globales.GuardarTiemposCarrera = true;
            m_curInventoryBuffer.dtTagTable.Rows.Clear();
            lvRealList.Items.Clear();
            ...
            try
            {
                ...
                sentencia.AppendLine("	CONCAT(C.APELLIDOPATERNO, ' ', C.APELLIDOMATERNO, ' ', C.NOMBRE) AS NOMBRECOMPLETO, ");
                sentencia.AppendLine("	B.CHIP, ");
                sentencia.AppendLine("	B.TIEMPO ");
                sentencia.AppendLine("FROM ");
                sentencia.AppendLine("	CARRERAS AS A ");
                sentencia.AppendLine("LEFT JOIN ");
                sentencia.AppendLine("	CARRERASDETALLE AS B ");
                sentencia.AppendLine("ON 	B.IDEMPRESA = A.IDEMPRESA ");
                sentencia.AppendLine("AND B.IDCARRERA = A.IdCarrera ");
                sentencia.AppendLine("LEFT JOIN ");
                sentencia.AppendLine("	COMPETIDORES AS C ");
                sentencia.AppendLine("ON 	C.IDEMPRESA = B.IDEMPRESA ");
                sentencia.AppendLine("AND C.IDCOMPETIDOR = B.IDCOMPETIDOR ");
                sentencia.AppendLine("WHERE ");
                sentencia.AppendLine("	A.IDEMPRESA = " + IdEmpresa);
                sentencia.AppendLine("AND A.IDCARRERA = " + idCarrera);
                ...
                while (reader.Read())
                {
                    ...
                    if (!(reader[indice] is DBNull))
                    {
                        DateTime tiempo = reader.GetDateTime(indice);
                        tiempoFormateado = tiempo.ToString("hh:mm:ss.fff tt");
                    }
                    
                    if (renglon == 0)
                    {
                        vista.txtFechaCarrera.Text = fecha.ToString("dd/MM/yyyy");
                    }

                    string[] row = new string[] { idCompetidor.ToString(), competidor, chip, tiempoFormateado };
                    grid.Rows.Add(row);

                    renglon++;
                }
                vista.tablaTiemposCorredores = grid;
                tablaTiempos = grid;
                vista.Show();
        ...
        public bool GuardarTiempos(string chip)
        {
            ...
            try
            {
                ...
                if (resultado)
                {
                    string query = "SELECT TIEMPO FROM CARRERASDETALLE WHERE IDEMPRESA =" + IdEmpresa + " AND IDCARRERA= " + idCarrera + " AND IDCATEGORIA =" + idCategoria + " AND CHIP = '" + chip + "'";
                    cmd = new MySqlCommand(query, mysqlCon);
                    DateTime tiempoBD = Convert.ToDateTime(cmd.ExecuteScalar());
                    tiempo = tiempoBD.ToString("hh:mm:ss.ffffff  tt");
                }
            }
            catch (Exception ex)
            {
        ...
        private void btnMostrarCarrera_Click(object sender, EventArgs e)
        {
        ...
        private void ActualizarTiempo(string chip)
        {
            try
            {
                if (GuardarTiempos(chip))
                {
                    foreach (DataGridViewRow row in tablaTiempos.Rows)
                    {
                        if (row.Cells[2].Value.ToString().Equals(chip))
                        {
                            if (row.Cells[3].Value == null || row.Cells[3].Value.ToString() == "")
                            {
                                row.Cells[3].Value = tiempo;
                                tiempo = "";
                                break;
                            }
                        }
                    ...
                    VistaCarreraTiempos vista = new VistaCarreraTiempos();
                    vista.tablaTiemposCorredores = tablaTiempos;
                }
            }
            catch (Exception ex)
            {
        ...
        private List<CarreraDetalle> LlenarDatosCarrera(int idCarrera)
        {
            List<CarreraDetalle> lista = new List<CarreraDetalle>();
            string connectionString = "SERVER=localhost;DATABASE=atletica;UID=root;PASSWORD=pecopeco1290;";
            MySqlConnection mysqlCon = new MySqlConnection(connectionString);
            mysqlCon.Open();
            try
            {
                ...
                sentencia.AppendLine("		A.IDEMPRESA = " + IdEmpresa);
                sentencia.AppendLine(" AND A.IDCARRERA = " + idCarrera);
                sentencia.AppendLine(" ORDER BY A.IDEMPRESA, A.IDCARRERA, A.ID, A.IDCATEGORIA");

                string query = sentencia.ToString();
                MySqlCommand cmd = new MySqlCommand(query, mysqlCon);
                MySqlDataReader reader = cmd.ExecuteReader();
                CarreraDetalle entidad = new CarreraDetalle();
                idCategoriasCarrera = new List<int>();
                categoriasCarrera = new List<string>();

                while (reader.Read())
                {
                    int indice = 0;
                    entidad = new CarreraDetalle();
                    entidad.IdCarreraDetalle = (reader[indice] is DBNull) ? 0 : reader.GetInt32(indice); indice++;
                    entidad.IdCompetidor = (reader[indice] is DBNull) ? 0 : reader.GetInt32(indice); indice++;
                    entidad.Competidor = (reader[indice] is DBNull) ? string.Empty : reader.GetString(indice); indice++;
                    entidad.IdCategoria = (reader[indice] is DBNull) ? 0 : reader.GetInt32(indice); indice++;
                    entidad.Categoria = (reader[indice] is DBNull) ? string.Empty : reader.GetString(indice); indice++;
                    entidad.Chip = (reader[indice] is DBNull) ? string.Empty : reader.GetString(indice); indice++;
                    lista.Add(entidad);

                    if (idCategoriasCarrera.FindIndex(s => s == entidad.IdCategoria) == -1)
                    {
                        idCategoriasCarrera.Add(entidad.IdCategoria);
                        categoriasCarrera.Add(entidad.Categoria);
                    }
                }
                return lista;
            }
        ...
        private void txtExportarExcel_Click(object sender, EventArgs e)
        {
            if (tablaCarreras.SelectedRows.Count == 0)
            {
                MessageBox.Show("Debe de seleccionar una carrera");
            ...
            {
                DataGridViewRow row = this.tablaCarreras.SelectedRows[0];
                int idCarrera = row.Cells["IdCarrera"] != null && row.Cells["IdCarrera"].Value != "" ? (int)row.Cells["IdCarrera"].Value : 0;
                string descripcion = row.Cells["DescripcionCarrera"] != null && row.Cells["DescripcionCarrera"].Value != "" ? row.Cells["DescripcionCarrera"].Value.ToString() : "";
                string fecha = row.Cells["Fecha"] != null && row.Cells["Fecha"].Value != null ? row.Cells["Fecha"].Value.ToString() : "";
                DateTime fechaConvertida = new DateTime();
                if (!string.IsNullOrEmpty(fecha))
                {
                    fechaConvertida = Convert.ToDateTime(fecha);
                }

                FileInfo newFile = new FileInfo("Tiempos de " + descripcion + " " + fechaConvertida.ToString("dd-MM-yyyy") + ".xlsx");
                // If any file exists in this directory having name 'Sample1.xlsx', then delete it
                if (newFile.Exists)
                {
                    newFile.Delete(); // ensures we create a new workbook
                    newFile = new FileInfo("Tiempos de " + descripcion + " " + fechaConvertida.ToString("dd-MM-yyyy") + ".xlsx");
                }
                using (var package = new ExcelPackage(newFile))
                {
                   

                    List<CarreraDetalle> lista = LlenarDatosCarrera(idCarrera);

                    if (categoriasCarrera != null && categoriasCarrera.Count > 0)
                    {
                        int pestana = 1;
                        foreach (string categoria in categoriasCarrera)
                        {
                            package.Workbook.Worksheets.Add(descripcion + " Categoria " + categoria);
                            ExcelWorksheet ws = package.Workbook.Worksheets[pestana];
                            ws.Name = descripcion + " Categoria " + categoria;
                            ...
                            ws.Column(1).Width = 15; //IdCarreraDetalle
                            ws.Column(2).Width = 40; //Competidor
                            ws.Column(3).Width = 20; //Categoria
                            ws.Column(4).Width = 35; //Chip
                            ws.Column(5).Width = 15; //Tiempo
                            ...
                            ws.Cells[3, 1].Value = descripcion;

                            //Merging cells and create a center heading for out table
                            ws.Cells[5, 1].Value = "CLAVE";
                            ws.Cells[5, 2].Value = "COMPETIDOR";
                            ws.Cells[5, 3].Value = "CATEGORIA";
                            ws.Cells[5, 4].Value = "CHIP";
                            ws.Cells[5, 5].Value = "TIEMPO";
                            var titulos = ws.Cells[5, 1, 5, 5];
                            ...
                            {
                                foreach (CarreraDetalle entidad in lista)
                                {
                                    //string tiempo = "";
                                    //if (!string.IsNullOrEmpty(entidad.Tiempo))
                                    //{
                                    //    DateTime time = Convert.ToDateTime(entidad.Tiempo);
                                    //    tiempo = time.ToString("hh:mm:ss tt");
                                    //}
                                    

                                    var dato = ws.Cells[renglon, 1, renglon, 5];
                                    ws.Cells[renglon, 1].Value = entidad.IdCarreraDetalle.ToString();
                                    ws.Cells[renglon, 1].Style.HorizontalAlignment = ExcelHorizontalAlignment.Center;
                                    ws.Cells[renglon, 2].Value = entidad.Competidor;
                                    ws.Cells[renglon, 3].Value = entidad.Categoria;
                                    ws.Cells[renglon, 4].Value = entidad.Chip;
                                    ws.Cells[renglon, 5].Value = tiempo;
        ...
        private void tablaCompetidores_CellContentDoubleClick(object sender, DataGridViewCellEventArgs e)
        {
            btnModificarCompetidor.PerformClick();
        }

        private void tablaCategorias_CellContentDoubleClick(object sender, DataGridViewCellEventArgs e)
        {
            btnModificarCategoria.PerformClick();
        }

        private void tablaCarreras_CellContentDoubleClick(object sender, DataGridViewCellEventArgs e)
        {
